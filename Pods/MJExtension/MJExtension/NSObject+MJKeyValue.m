...
{
    // 获得JSON对象
    keyValues = [keyValues mj_JSONObject]; //输入对象转JSON
    ...
    [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
        @try {
            ...
            // 1.取出属性值
            id value;
            NSArray *propertyKeyses = [property propertyKeysForClass:clazz]; //取出类的所有成员变量 ？？？
            ...
            Class objectClass = [property objectClassInArrayForClass:[self class]];//集合中的模型类
            ...
            if (!type.isFromFoundation && propertyClass) { // 模型属性 自定义模型
...
- (NSMutableDictionary *)mj_keyValuesWithKeys:(NSArray *)keys ignoredKeys:(NSArray *)ignoredKeys
{
    ...
    [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
        @try {
            ...
            MJPropertyType *type = property.type;
            Class propertyClass = type.typeClass;
            if (!type.isFromFoundation && propertyClass) { //非Foundation框架和基本数据类型
                value = [value mj_keyValues]; //自定义类 --> 模型转字典
            ...
            // 4.赋值    //模型转字典时，字典的key是否参考replacedKeyFromPropertyName
            if ([clazz mj_isReferenceReplacedKeyWhenCreatingKeyValues]) {//用设置的字典Key替代模型属性名称
                ...
                keyValues[property.name] = value; //用属性名作为Key
            }
        } @catch (NSException *exception) {
            MJExtensionBuildError([self class], exception.reason);
            MJExtensionLog(@"%@", exception);
...
{
    if ([self isKindOfClass:[NSString class]]) {
        return [NSJSONSerialization JSONObjectWithData:[((NSString *)self) dataUsingEncoding:NSUTF8StringEncoding] options:kNilOptions error:nil];//字符串转JSON
    } else if ([self isKindOfClass:[NSData class]]) { //NSData转JSON
