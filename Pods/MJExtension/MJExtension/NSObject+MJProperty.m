    ...
    if (cachedProperties == nil) {
        ...
        [self mj_enumerateClasses:^(__unsafe_unretained Class c, BOOL *stop) {
            ...
            for (unsigned int i = 0; i<outCount; i++) {
                ...
                property.srcClass = c;//设置属性的来源
                [property setOriginKey:[self propertyKey:property.name] forClass:self]; //字典中的Key
                [property setObjectClassInArray:[self propertyObjectClassInArray:property.name] forClass:self];//存储数组中模型的Class
